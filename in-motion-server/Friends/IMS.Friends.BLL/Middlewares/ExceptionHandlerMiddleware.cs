...
public class ExceptionHandlerMiddleware: IMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, RequestDelegate next)
    {
        ...
        catch (InvalidFriendshipActionException exception)
        {
            ...
            await SendErrorResponse(context, StatusCodes.Status400BadRequest, "User Guid string is empty", nameof(exception));
        }
        catch (RabbitMqException exception)
        {
            _logger.LogWarning( exception, "RabbitMq request failure, {Message}", exception.Message);
            await SendErrorResponse(context, StatusCodes.Status500InternalServerError, "User with this provider exists", nameof(exception));
        }
