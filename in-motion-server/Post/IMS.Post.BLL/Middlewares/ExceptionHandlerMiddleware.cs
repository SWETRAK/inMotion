...
public class ExceptionHandlerMiddleware : IMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, RequestDelegate next)
    {
        ...
        catch (InvalidGuidStringException exception)
        {
            ...
            await SendErrorResponse(context, StatusCodes.Status401Unauthorized, "User try to authenticate with incorrect google token", exception.GetType().ToString());
        }
        catch (PostNotFoundException exception)
        {
            ...
            await SendErrorResponse(context, StatusCodes.Status404NotFound, "InternalServerError",
